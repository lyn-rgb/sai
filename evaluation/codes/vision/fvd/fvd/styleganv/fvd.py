...
def load_i3d_pretrained(device=torch.device('cpu'), weights_path=None):
    ...
    filepath = weights_path or os.path.join(os.path.dirname(os.path.abspath(__file__)), 'i3d_torchscript.pt')
    print('Loading model from: %s'%filepath)
    if not os.path.exists(filepath):
        if weights_path:
            raise FileNotFoundError(f"Configured StyleGANV I3D weights do not exist: {filepath}")
