...
if [[ -n "${LORA_PATH:-}" || -n "${DURATION_SECONDS:-}" || -n "${FPS:-}" || -n "${OUTPUT_DIR:-}" || -n "${SKIP_EXISTING_OUTPUTS:-}" || -n "${USE_REFERENCE_TOKENS:-}" || -n "${USE_IP_EMBEDDINGS:-}" || -n "${CROP_FACE:-}" ]]; then
  RUN_CONFIG="$(mktemp "${TMPDIR:-/tmp}/full_ovi_testdata_5s.XXXXXX.yaml")"
  "$PYTHON_BIN" - "$CONFIG_FILE" "$RUN_CONFIG" "${LORA_PATH:-}" "${DURATION_SECONDS:-}" "${FPS:-}" "${OUTPUT_DIR:-}" "${SKIP_EXISTING_OUTPUTS:-}" "${USE_REFERENCE_TOKENS:-}" "${USE_IP_EMBEDDINGS:-}" "${CROP_FACE:-}" <<'PY'
from pathlib import Path
import sys

src, dst = map(Path, sys.argv[1:3])
lora_path, duration_seconds, fps, output_dir, skip_existing_outputs, use_reference_tokens, use_ip_embeddings, crop_face = sys.argv[3:11]
updates = {}
if lora_path:
...
if [[ -n "$CONFIG_LORA_PATH" && ! -f "$CONFIG_LORA_PATH" ]]; then
  ...
  echo "Set a valid path with: LORA_PATH=/path/to/step.safetensors bash inference_full_ovi_testdata_5s.sh" >&2
  exit 1
fi
