...
def load_i3d_pretrained(device=torch.device('cpu'), weights_path=None):
    ...
    filepath = weights_path or os.path.join(os.path.dirname(os.path.abspath(__file__)), 'i3d_pretrained_400.pt')
    print(f"Loading VideoGPT I3D weights from: {filepath}")
    if not os.path.exists(filepath):
        if weights_path:
            raise FileNotFoundError(f"Configured VideoGPT I3D weights do not exist: {filepath}")
