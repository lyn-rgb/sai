...
class Cropper(object):
    def __init__(self, **kwargs) -> None:
        ...
        cuda_provider_options = kwargs.get("cuda_provider_options", None)
        print(f"==> Cropper device: {device_id}")
        ...
        if flag_force_cpu:
            ...
            try:
                if torch.backends.mps.is_available():
                    ...
                    device = "cuda"
                    face_analysis_wrapper_provider = [("CUDAExecutionProvider", cuda_provider_options)]
            except:
                    device = "cuda"
                    face_analysis_wrapper_provider = [("CUDAExecutionProvider", cuda_provider_options)]
    ...
    def crop_source_image(self, img_rgb_: np.ndarray, crop_cfg: CropConfig, crop_size: Union[int, None] = None):
        ...
        # crop the face
        crop_size = crop_size if crop_size is not None else crop_cfg.dsize
        ret_dct = crop_image(
            img_rgb,  # ndarray
            lmk,  # 106x2 or Nx2
            dsize=crop_size,
    ...
    def crop_source_video(self, source_rgb_lst, crop_cfg: CropConfig = None, **kwargs):
    ...
    def crop_driving_video(self, driving_rgb_lst, **kwargs):
        ...
        trajectory = Trajectory()
        direction = kwargs.get("direction", "large-small")
        flag_rot = kwargs.get("flag_rot", False)
