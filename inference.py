...
def main(config, args): 
    ...
    for _, (text_prompt, image_path, ip_image_path, ip_audio_path) in tqdm(enumerate(this_rank_eval_data)):
        ...
        if ip_image_path is not None:
            if not os.path.isfile(ip_image_path):
                print(f"IP Image {ip_image_path} not exists, using `None` instead")
                ip_image_path = None
                
        if ip_audio_path is not None:
            if not os.path.isfile(ip_audio_path):
                print(f"IP Audio {ip_audio_path} not exists, using `None` instead")
                ip_audio_path = None
        ...
        for idx in range(config.get("each_example_n_times", 1)):
            ...
            if sp_rank == 0:
                ...
                output_path = os.path.join(output_dir, f"crop-{crop_face}_ip-image-{ip_image_path is not None}_ip-audio-{ip_audio_path is not None}_{formatted_prompt}_{'x'.join(map(str, video_frame_height_width))}_{seed+idx}_{global_rank}.mp4")
