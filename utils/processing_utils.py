...
def preprocess_audio_tensor(audio_path, device, target_dtype, sr=16000, clip_len=None, normalize=True):
    ...
    if clip_len is not None:
        ...
        if len(wave_data) < clip_len:
            # pad like the training dataset does, otherwise a short reference would produce fewer
            # reference tokens at inference than the model was trained with
            wave_data = np.pad(wave_data, (0, clip_len - len(wave_data)))
...
def split_reference_paths(value) -> list:
    """One reference path per person, semicolon separated.

    A single path (no separator) is the one-person case, so `validate_and_process_user_prompt`
    always returns a list of lists and callers do not need to care about the reference count.
    """
    if value is None:
        return []
...
def validate_and_process_user_prompt(text_prompt: str, image_path: str = None, ip_image_path: str = None, ip_audio_path: str = None, mode: str = "id2v") -> str:
    ...
    if os.path.isfile(text_prompt):
        ...
        # reference persons: `ip_image_paths` / `ip_audio_paths` hold one path per person
        # (semicolon separated), with the singular columns still supported for one-person runs
        if mode == "id2v" and 'ip_image_paths' in df.keys():
            ip_image_paths = [split_reference_paths(p) for p in df["ip_image_paths"]]
        elif mode == "id2v" and 'ip_image_path' in df.keys():
            ip_image_paths = [split_reference_paths(p) for p in df["ip_image_path"]]
        else:
            print("Warning: ip_image_path(s) was not found, assuming i2v or t2v or t2i2v mode...")
            ip_image_paths = [[] for _ in text_prompts]
        print(f"ip images: {ip_image_paths}")
        assert all(os.path.isfile(p) for refs in ip_image_paths for p in refs), "One or more ip image paths in the TSV file do not exist."

        if mode == "id2v" and 'ip_audio_paths' in df.keys():
            ip_audio_paths = [split_reference_paths(p) for p in df["ip_audio_paths"]]
        elif mode == "id2v" and 'ip_audio_path' in df.keys():
            ip_audio_paths = [split_reference_paths(p) for p in df["ip_audio_path"]]
        else:
            print("Warning: ip_audio_path(s) was not found, assuming i2v or t2v or t2i2v mode...")
            ip_audio_paths = [[] for _ in text_prompts]
        assert all(os.path.isfile(p) for refs in ip_audio_paths for p in refs), "One or more ip audio paths in the TSV file do not exist."
    else:
        ...
        ip_image_paths = [split_reference_paths(ip_image_path)]
        ip_audio_paths = [split_reference_paths(ip_audio_path)]
